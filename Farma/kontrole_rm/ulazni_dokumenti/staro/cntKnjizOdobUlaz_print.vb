...
Public Class cntKnjizOdobUlaz_print
    ...
    Shared sql_start As String = _
                ...
                "rm_knjizno_odobrenje_u_head.knjod_ulaz_broj, rm_knjizno_odobrenje_u_head.knjod_ulaz_datum, rm_knjizno_odobrenje_u_head.knjod_ulaz_ukupno, " & _
                "rm_knjizno_odobrenje_u_head.knjod_ulaz_ztroskovi, rm_knjizno_odobrenje_u_head.knjod_ulaz_rabat,  " & _
                "rm_knjizno_odobrenje_u_head.knjod_ulaz_razlika_uceni, rm_knjizno_odobrenje_u_head.knjod_ulaz_pdv_osnovica,  " & _
                "rm_knjizno_odobrenje_u_head.knjod_ulaz_pdv, rm_knjizno_odobrenje_u_head.knjod_ulaz_svega, rm_knjizno_odobrenje_u_head.knjod_ulaz_zakljucena,  " & _
                "rm_magacin.magacin_sifra, rm_magacin.magacin_naziv, app_partneri.partner_sifra,  " & _
                "app_partneri.partner_naziv " & _
                "FROM rm_knjizno_odobrenje_u_head LEFT OUTER JOIN " & _
                "rm_magacin ON rm_knjizno_odobrenje_u_head.id_magacina = rm_magacin.id_magacin LEFT OUTER JOIN " & _
                "app_partneri ON rm_knjizno_odobrenje_u_head.id_dobavljac = app_partneri.id_partner"
    ...
    Private Sub cntKnjizOdobUlaz_print_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        popuni_Magacine()
        popuni_dobavljace()
    ...
    Private Sub filter()
        ...
        If upit <> "" Then
            sql = sql_start & " WHERE " & upit & " ORDER BY rm_knjizno_odobrenje_ulaz_head.knjod_ulaz_datum DESC," & _
                                                 " rm_knjizno_odobrenje_ulaz_head.knjod_ulaz_broj DESC"
    ...
    Shared Sub Lista()
        ...
        If sql <> sql_start Then
            ...
            If CN.State = ConnectionState.Open Then
                ...
                While DR.Read
                    Dim podatak As New ListViewItem(CStr(DR.Item("knjod_ulaz_broj")))

                    podatak.SubItems.Add(DR.Item("knjod_ulaz_datum"))
                    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("partner_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("knjod_ulaz_ukupno").ToString)
                    podatak.SubItems.Add(da_ne(DR.Item("knjod_ulaz_zakljucena").ToString))
    ...
    Private Sub rbtZaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtZaklj.CheckedChanged
        Select Case rbtZaklj.Checked
            Case True
                rbtNezaklj.Checked = False
                upit_zakljcene = "rm_knjizno_odobrenje_u_head.knjod_ulaz_zakljucena = 1"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub rbtNezaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtNezaklj.CheckedChanged
        Select Case rbtNezaklj.Checked
            Case True
                rbtZaklj.Checked = False
                upit_zakljcene = "rm_knjizno_odobrenje_u_head.knjod_ulaz_zakljucena = 0"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub chkSve_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSve.CheckedChanged
        ...
        Select Case chkSve.CheckState
            Case CheckState.Checked
                ...
                sql = sql_start + " ORDER BY rm_knjizno_odobrenje_u_head.knjod_ulaz_datum DESC"
                filter()
                'Lista()
    ...
    Private Sub txtBroj_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBroj.KeyPress
        If e.KeyChar = Chr(13) Then
            If txtBroj.Text <> "" Then
                upit_broj = "rm_knjizno_odobrenje_u_head.knjod_ulaz_broj = " & txtBroj.Text '& "%'"
            Else
                upit_broj = ""
            End If
            filter()
    ...
    Private Sub datDatum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles datDatum.KeyPress
        If e.KeyChar = Chr(13) Then
            upit_datum = "rm_knjizno_odobrenje_u_head.knjod_ulaz_datum = '" & _
