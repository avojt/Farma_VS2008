...
Public Class cntKalkulacija_search
    ...
    Shared sql_start As String = _
                ...
                "rm_kalkulacija_head.kalk_broj, rm_kalkulacija_head.kalk_datum, rm_kalkulacija_head.kalk_ukupno, " & _
                "rm_kalkulacija_head.kalk_ztroskovi, rm_kalkulacija_head.kalk_rabat,  " & _
                "rm_kalkulacija_head.kalk_razlika_uceni, rm_kalkulacija_head.kalk_pdv_osnovica,  " & _
                "rm_kalkulacija_head.kalk_pdv, rm_kalkulacija_head.kalk_svega, rm_kalkulacija_head.kalk_zakljucena,  " & _
                "rm_magacin.magacin_sifra, rm_magacin.magacin_naziv, app_partneri.partner_sifra,  " & _
                "app_partneri.partner_naziv " & _
                "FROM rm_kalkulacija_head LEFT OUTER JOIN " & _
                "rm_magacin ON rm_kalkulacija_head.id_magacina = rm_magacin.id_magacin LEFT OUTER JOIN " & _
                "app_partneri ON rm_kalkulacija_head.id_dobavljac = app_partneri.id_partner"
    ...
    Private Sub cntKalkulacija_search_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub filter()
        ...
        If upit <> "" Then
            sql = sql_start & " WHERE " & upit & " ORDER BY rm_kalkulacija_head.kalk_datum DESC," & _
                                                 " rm_kalkulacija_head.kalk_broj DESC"
    ...
    Shared Sub Lista()
        ...
        If sql <> sql_start Then
            ...
            If CN.State = ConnectionState.Open Then
                ...
                While DR.Read
                    Dim podatak As New ListViewItem(CStr(DR.Item("kalk_broj")))

                    podatak.SubItems.Add(DR.Item("kalk_datum"))
                    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("partner_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("kalk_pdv_osnovica").ToString)
                    podatak.SubItems.Add(da_ne(DR.Item("kalk_zakljucena").ToString))
    ...
    Private Sub rbtZaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtZaklj.CheckedChanged
        Select Case rbtZaklj.Checked
            Case True
                rbtNezaklj.Checked = False
                upit_zakljcene = "rm_kalkulacija_head.kalk_zakljucena = 1"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub rbtNezaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtNezaklj.CheckedChanged
        Select Case rbtNezaklj.Checked
            Case True
                rbtZaklj.Checked = False
                upit_zakljcene = "rm_kalkulacija_head.kalk_zakljucena = 0"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub txtBroj_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBroj.KeyPress
        If e.KeyChar = Chr(13) Then
            If txtBroj.Text <> "" Then
                upit_broj = "rm_kalkulacija_head.kalk_broj = " & txtBroj.Text '& "%'"
            Else
                upit_broj = ""
            End If
            filter()
    ...
    Private Sub datDatum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles datDatum.KeyPress
        If e.KeyChar = Chr(13) Then
            upit_datum = "rm_kalkulacija_head.kalk_datum = '" & _
