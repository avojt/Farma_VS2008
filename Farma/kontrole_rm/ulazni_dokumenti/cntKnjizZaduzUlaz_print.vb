...
Public Class cntKnjizZaduzUlaz_print
    ...
    Shared sql_start As String = _
                ...
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_broj, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_datum, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_ukupno, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_ztroskovi, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_rabat,  " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_razlika_uceni, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_pdv_osnovica,  " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_pdv, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_svega, " & _
                "rm_knjizno_zaduzenje_ulaz_head.knjzadU_zakljucena,  " & _
                "rm_magacin.magacin_sifra, " & _
                "rm_magacin.magacin_naziv, " & _
                "app_partneri.partner_sifra,  " & _
                "app_partneri.partner_naziv " & _
                "FROM rm_knjizno_zaduzenje_ulaz_head LEFT OUTER JOIN " & _
                "rm_magacin ON rm_knjizno_zaduzenje_ulaz_head.knjzadU_.id_magacina = rm_magacin.id_magacin LEFT OUTER JOIN " & _
                "app_partneri ON rm_knjizno_zaduzenje_ulaz_head.id_dobavljac = app_partneri.id_partner"
    ...
    Private Sub cntKnjizZaduzUlaz_print_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed

    End Sub

    Private Sub cntKnjizZaduzUlaz_print_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        popuni_Magacine()
        popuni_dobavljace()
    ...
    Private Sub filter()
        ...
        If upit <> "" Then
            sql = sql_start & " WHERE " & upit & " ORDER BY rm_knjizno_zaduzenje_ulaz_head.knjzadU_datum DESC," & _
                                                 " rm_knjizno_zaduzenje_ulaz_head.knjzadU_broj DESC"
    ...
    Shared Sub Lista()
        ...
        If sql <> sql_start Then
            ...
            If CN.State = ConnectionState.Open Then
                ...
                While DR.Read
                    Dim podatak As New ListViewItem(CStr(DR.Item("knjzadU_broj")))

                    podatak.SubItems.Add(DR.Item("knjzadU_datum"))
                    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("partner_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("knjzadU_pdv_osnovica").ToString)
                    podatak.SubItems.Add(da_ne(DR.Item("knjzadU_zakljucena").ToString))
    ...
    Private Sub rbtZaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtZaklj.CheckedChanged
        Select Case rbtZaklj.Checked
            Case True
                rbtNezaklj.Checked = False
                upit_zakljcene = "rm_knjizno_zaduzenje_ulaz_head.knjzadU_zakljucena = 1"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub rbtNezaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtNezaklj.CheckedChanged
        Select Case rbtNezaklj.Checked
            Case True
                rbtZaklj.Checked = False
                upit_zakljcene = "rm_knjizno_zaduzenje_ulaz_head.knjzadU_zakljucena = 0"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub chkSve_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSve.CheckedChanged
        ...
        Select Case chkSve.CheckState
            Case CheckState.Checked
                ...
                sql = sql_start + " ORDER BY rm_knjizno_zaduzenje_ulaz_head.knjzadU_datum DESC"
                filter()
                'Lista()
    ...
    Private Sub txtBroj_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBroj.KeyPress
        If e.KeyChar = Chr(13) Then
            If txtBroj.Text <> "" Then
                upit_broj = "rm_knjizno_zaduzenje_ulaz_head.knjzadU_broj = " & txtBroj.Text '& "%'"
            Else
                upit_broj = ""
            End If
            filter()
    ...
    Private Sub datDatum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles datDatum.KeyPress
        If e.KeyChar = Chr(13) Then
            upit_datum = "rm_knjizno_zaduzenje_ulaz_head.knjzadU_datum = '" & _
