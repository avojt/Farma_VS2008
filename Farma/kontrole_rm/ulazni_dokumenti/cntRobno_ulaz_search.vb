...
Public Class cntRobno_ulaz_search
    ...
    Private Sub cntRobno_search_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        popuni_Magacine()
        popuni_dobavljace()
    ...
    Private Sub popuni_vrste_dokumenta()
        ...
        If CN.State = ConnectionState.Open Then
            With CM
                ...
                .CommandText = "select dbo.app_vrste_dokumenata.* from dbo.app_vrste_dokumenata " & _
                                "where vrsta_dok_strana_knjizenja = 'DUG' " & _
                                "and ostala_dokumenta = 0 order by vrsta_dok_naziv "
                DR = .ExecuteReader
            End With
            Do While DR.Read
    ...
    Private Sub filter()
        ...
        If rbtNezaklj.Checked = True Then
            upit_zakljcene = "rm_ulazni_dokument_head.dok_zakljucen = 0"
        End If

        If rbtZaklj.Checked = True Then
            upit_zakljcene = "rm_ulazni_dokument_head.dok_zakljucen = 1"
        End If

        If Not _sve Then
            If cmbVrstaDok.Text <> "" Then
                sql += " where rm_ulazni_dokument_head.id_vrsta_dokumenta = " & mRob_Dokument.dokumenta_id
                'Else
                'sql += " where rm_ulazni_dokument_head.id_vrsta_dokumenta = " & mRob_Dokument.dokumenta_id
            End If
            ...
            If chkBroj.Checked Then
                If txtBroj.Text <> "" Then
                    upit_broj = "rm_ulazni_dokument_head.dok_broj = " & txtBroj.Text '& "%'"
                Else
                    upit_broj = ""
                End If
            End If
            ...
            If chkDatum.Checked Then
                upit_datum = "rm_ulazni_dokument_head.dok_datum = '" & _
    ...
    Shared Sub Lista()
        ...
        If sql <> "" Then
            ...
            If CN.State = ConnectionState.Open Then
                ...
                While DR.Read
                    Dim podatak As New ListViewItem(CStr(DR.Item(_dok_kolone(0) & "broj")))
                    'If mRob_Dokument.tabela = Imena.tabele.rm_nivelacije_head.ToString Then
                    ...
                    '    podatak.SubItems.Add(DR.Item("id_nivelacija").ToString)

                    'ElseIf mRob_Dokument.tabela = Imena.tabele.rm_popis_head.ToString Then
                    '    podatak.SubItems.Add(DR.Item("pop_datum").ToString)
                    '    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    '    podatak.SubItems.Add(DR.Item("pop_vrednost").ToString)
                    '    podatak.SubItems.Add(DR.Item("id_popis").ToString)

                    'Else
                    podatak.SubItems.Add(DR.Item("dok_datum"))
                    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("partner_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("dok_pdv_osnovica").ToString)
                    podatak.SubItems.Add(da_ne(DR.Item("dok_zakljucen")))
                    If _sve Then
                        podatak.SubItems.Add(DR.Item("vrsta_dok_naziv").ToString)
                    End If
                    podatak.SubItems.Add(DR.Item("id_dokument").ToString)
                    'End If
    ...
    '            upit_zakljcene = mRob_Dokument.KonamdTekst & "_head." & _dok_kolone(0) & "zakljucen = 1"
    '        Case False
    '            upit_zakljcene = ""
    '    End Select
    '    'filter()
    ...
    '        Case True
    '            rbtZaklj.Checked = False
    '            upit_zakljcene = mRob_Dokument.KonamdTekst & "_head." & _dok_kolone(0) & "zakljucen = 0"
    '        Case False
    '            upit_zakljcene = ""
    '    End Select
    '    'filter()
    ...
    Private Sub chkSve_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSve.CheckedChanged
        ...
        Select Case chkSve.CheckState
            Case CheckState.Checked
                ...
                sql = sql_start + " ORDER BY " & mRob_Dokument.KonamdTekst & "_head." & _dok_kolone(0) & "datum DESC"
                filter()
                'Lista()
                _sve = True
    ...
    Private Sub txtBroj_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBroj.KeyPress
        If e.KeyChar = Chr(13) Then
            If txtBroj.Text <> "" Then
                upit_broj = mRob_Dokument.KonamdTekst & "_head." & _dok_kolone(0) & "broj = " & txtBroj.Text '& "%'"
            Else
                upit_broj = ""
            End If
            filter()
    ...
    Private Sub datDatum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles datDatum.KeyPress
        If e.KeyChar = Chr(13) Then
            upit_datum = mRob_Dokument.KonamdTekst & "_head." & _dok_kolone(0) & "datum = '" & _
                                 datDatum.Value.Month.ToString & "/" & _
                                 datDatum.Value.Day.ToString & "/" & _
    ...
    Private Sub cmbVrstaDok_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbVrstaDok.SelectedIndexChanged
        ...
        '                 "dbo.rm_nivelacije_head.id_nivelacija, dbo.rm_nivelacije_head.broj, dbo.rm_nivelacije_head.datum, " & _
        ...
        '    sql_start = "SELECT DISTINCT " & _
        '                   "dbo.rm_popis_head.id_popis, dbo.rm_popis_head.pop_broj, dbo.rm_popis_head.pop_datum, " & _
        ...
        sql_start = "SELECT " & _
            "rm_ulazni_dokument_head.id_dokument, " & _
            ...
            "FROM rm_ulazni_dokument_head " & _
            "LEFT OUTER JOIN rm_magacin ON rm_ulazni_dokument_head.id_magacina = rm_magacin.id_magacin " & _
            "LEFT OUTER JOIN app_partneri ON rm_ulazni_dokument_head.id_partner = app_partneri.id_partner " & _
            "LEFT OUTER JOIN app_vrste_dokumenata ON rm_ulazni_dokument_head.id_vrsta_dokumenta = app_vrste_dokumenata.id_vrsta_dok "
        'End If
...
#Region "STAMPANJE"
    Shared Sub prn()
        If _lista.SelectedItems.Count > 0 Then
            selektuj_dokument_ul(_lista.SelectedItems(0).SubItems(0).Text, Selekcija.po_sifri)
            dokument_ul_print()
