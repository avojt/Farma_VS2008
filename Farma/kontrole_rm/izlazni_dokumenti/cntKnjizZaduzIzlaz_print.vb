...
Public Class cntKnjizZaduzIzlaz_print
    ...
    Shared sql_start As String = _
                "SELECT DISTINCT TOP (100) PERCENT " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_broj, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_datum, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_ukupno, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_ztroskovi, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_rabat, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_razlika_uceni, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_pdv_osnovica,  " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_pdv, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_svega, " & _
                    "rm_knjizno_zaduzenje_izlaz_head.kz_iz_zakljucena, " & _
                ...
                "FROM rm_knjizno_zaduzenje_izlaz_head " & _
                "LEFT OUTER JOIN rm_magacin ON rm_knjizno_zaduzenje_izlaz_head.id_magacina = rm_magacin.id_magacin " & _
                "LEFT OUTER JOIN app_organizacione_jedinice ON rm_knjizno_zaduzenje_izlaz_head.id_dobavljac = app_organizacione_jedinice.id_orgjed"
    ...
    Private Sub cntKnjizZaduzIzlaz_print_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        popuni_Magacine()
        popuni_OJ()
    ...
    Private Sub popuni_OJ()
        ...
        If CN.State = ConnectionState.Open Then
            ...
            End With
            Do While DR.Read
                cmbOJ.Items.Add(DR.Item("partner_naziv"))
            Loop
            DR.Close()
        End If
    ...
    Private Sub filter()
        ...
        If upit <> "" Then
            sql = sql_start & " WHERE " & upit & " ORDER BY rm_knjizno_zaduzenje_izlaz_head.kz_iz_datum DESC," & _
                                                 " rm_knjizno_zaduzenje_izlaz_head.kz_iz_broj DESC"
    ...
    Shared Sub Lista()
        ...
        If sql <> sql_start Then
            ...
            If CN.State = ConnectionState.Open Then
                ...
                While DR.Read
                    Dim podatak As New ListViewItem(CStr(DR.Item("kz_iz_broj")))

                    podatak.SubItems.Add(DR.Item("kz_iz_datum"))
                    podatak.SubItems.Add(DR.Item("magacin_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("oj_naziv").ToString)
                    podatak.SubItems.Add(DR.Item("kz_iz_svega").ToString)
                    podatak.SubItems.Add(da_ne(DR.Item("kz_iz_zakljucena").ToString))
    ...
    Private Sub rbtZaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtZaklj.CheckedChanged
        Select Case rbtZaklj.Checked
            Case True
                rbtNezaklj.Checked = False
                upit_zakljcene = "rm_knjizno_zaduzenje_izlaz_head.kz_iz_zakljucena = 1"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub rbtNezaklj_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtNezaklj.CheckedChanged
        Select Case rbtNezaklj.Checked
            Case True
                rbtZaklj.Checked = False
                upit_zakljcene = "rm_knjizno_zaduzenje_izlaz_head.kz_iz_zakljucena = 0"
            Case False
                upit_zakljcene = ""
        End Select
        filter()
    ...
    Private Sub chkSve_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkSve.CheckedChanged
        ...
        Select Case chkSve.CheckState
            Case CheckState.Checked
                ...
                sql = sql_start + " ORDER BY rm_knjizno_zaduzenje_izlaz_head.kz_iz_datum DESC"
                filter()
                'Lista()
    ...
    Private Sub txtBroj_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBroj.KeyPress
        If e.KeyChar = Chr(13) Then
            If txtBroj.Text <> "" Then
                upit_broj = "rm_knjizno_zaduzenje_izlaz_head.ko_iz_broj = " & txtBroj.Text '& "%'"
            Else
                upit_broj = ""
            End If
            filter()
    ...
    Private Sub datDatum_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles datDatum.KeyPress
        If e.KeyChar = Chr(13) Then
            upit_datum = "rm_knjizno_zaduzenje_izlaz_head.ko_iz_datum = '" & _
    ...
    Private Sub cmbDobavljac_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbOJ.KeyPress
        If e.KeyChar = Chr(13) Then
            If cmbOJ.Text <> "" Then
                upit_dobavljac = "app_partneri.partner_naziv = N'" & cmbOJ.Text & "'"
            Else
                upit_dobavljac = ""
            End If
            filter()
        End If
    End Sub
    Private Sub cmbDobavljac_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbOJ.SelectedIndexChanged
        If cmbOJ.Text <> "" Then
            upit_dobavljac = "app_partneri.partner_naziv = N'" & cmbOJ.Text & "'"
        Else
            upit_dobavljac = ""
